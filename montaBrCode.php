...
function montaBrCode($dadosPix, $tipo, $pagoUmaVez, $cidade, $cep, $valorLivre, $tamanhoQrCode)
{

  $payload = getPayload($dadosPix["loc"]["location"]);

  if (!isset($payload["mensagem"])) { // Se o retorno da consulta do payload apresentar erro, não irá montar o QR Code e mostrará a mensagem
    ...
    if ($tipo === "dinamico") {
      $location = str_replace("https://", "", $dadosPix["loc"]["location"]); // URL payload do PSP do recebedor que contém as informações da cobrança
    ...
    $payloadBrCode .= ($valorLivre === true) ? preencheCampo('0') : ($tipo === "dinamico") ? preencheCampo($payload["valor"]["original"]) : preencheCampo($dadosPix["valor"]["original"]);

    $payloadBrCode .= '58' . preencheCampo($country_code); // [obrigatório] “BR” – Código de país ISO3166-1 alpha 2

    $payloadBrCode .= '59'; // [opcional] Nome do beneficiário/recebedor. Máximo: 25 caracteres.
